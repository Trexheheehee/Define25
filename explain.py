...
class TailAssignmentEnv:
    ...
    def step(self, action, epsilon):
        ...
        if action not in available_aircraft:
            ...
            explanation = {'reason': 'Invalid assignment', 'breakdown': {}}
            self.assignments[flight_id] = {'aircraft': None, 'explanation': explanation}
        else:
            # Valid assignment
            aircraft_reg = action
            reward, breakdown = self._calculate_reward(flight, aircraft_reg)
            explanation = {
                'reason': 'Optimal choice' if random.random() >= epsilon else 'Random exploration',
                'breakdown': breakdown
            }
            self.assignments[flight_id] = {'aircraft': aircraft_reg, 'explanation': explanation}
    ...
    def _calculate_reward(self, flight, aircraft_reg):
        ...
        if 'Physical Seating capacity' in flight and 'Seating capacity' in aircraft:
            req = flight['Physical Seating capacity']
            avail = aircraft['Seating capacity']
            
            if avail >= req:
                efficiency = 1.0 - ((avail - req) / avail)
                breakdown['capacity_efficiency'] = 5 * efficiency
            else:
        ...
        for episode in range(episodes):
            ...
            while not done:
                ...
                if action is None:
                    ...
                    next_state, reward, done, _ = self.env.step(None, self.epsilon)
                else:
                    next_state, reward, done, _ = self.env.step(action, self.epsilon)
        ...
        if flights_file is not None and aircraft_file is not None:
            try:
                ...
                if st.button("Train Model"):
                    ...
                    # Show assignments with explanations
                    ...
                    for flight_idx, flight in flights_df.iterrows():
                        ...
                        assignment = assignments.get(flight_id, {})
                        
                        col1, col2 = st.columns([3, 1])
                        with col1:
                            if 'Departure Station' in flight and 'Arrival Station' in flight:
                                st.markdown(f"**{flight_id}** ({flight['Departure Station']} → {flight['Arrival Station']})")
                            else:
                                st.markdown(f"**{flight_id}** (Departure/Arrival Station info missing)")
                        with col2:
                            st.markdown(f"Assigned: **{assignment.get('aircraft', 'None')}**")
                        
                        with st.expander("Explanation"):
                            if 'explanation' in assignment:
                                expl = assignment['explanation']
                                st.write(f"**Decision Type:** {expl['reason']}")
                                
                                # Human-readable explanation
                                explanation_text = ""
                                if expl['reason'] == 'Optimal choice':
                                    explanation_text += "The aircraft was chosen because it was the optimal choice based on the Q-learning model."
                                else:
                                    explanation_text += "The aircraft was chosen randomly as part of the exploration strategy."
                                
                                explanation_text += "\n\n**Here's a breakdown of the factors that influenced the assignment:**\n"
                                
                                if 'type_match' in expl['breakdown'] and expl['breakdown']['type_match'] > 0:
                                    explanation_text += f"- **Type Match:** The aircraft type matched the required aircraft type for this flight, contributing {expl['breakdown']['type_match']:.1f} points.\n"
                                else:
                                    explanation_text += "- **Type Match:** The aircraft type did not perfectly match the required aircraft type for this flight.\n"
                                    
                                if 'capacity_efficiency' in expl['breakdown'] and expl['breakdown']['capacity_efficiency'] > 0:
                                    explanation_text += f"- **Capacity Efficiency:** The aircraft's seating capacity was well-suited to the flight's passenger capacity, resulting in a capacity efficiency score of {expl['breakdown']['capacity_efficiency']:.1f}.\n"
                                else:
                                    explanation_text += "- **Capacity Efficiency:** The aircraft's seating capacity was not ideally suited to the flight's passenger capacity.\n"
                                    
                                if 'penalty' in expl['breakdown'] and expl['breakdown']['penalty'] < 0:
                                    explanation_text += f"- **Penalty:** A penalty of {expl['breakdown']['penalty']:.1f} was applied, likely due to the aircraft's capacity being insufficient for the flight.\n"
                                
                                if 'base' in expl['breakdown']:
                                    explanation_text += f"\nThe base score was {expl['breakdown']['base']:.1f}."
